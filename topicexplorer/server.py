...
def docs(docs=None, q=None):
    ...
    try:
        ...
        if request.query.random:
            docs = [random.choice(lda_v.corpus.view_metadata(
                context_type)[doc_label_name(context_type)])]
    except:
        pass
...
def get_docs(docs=None, id_as_key=False, query=None):
    ...
    if docs:
        ...
        ids = [lda_v.corpus.meta_int(
            context_type, {doc_label_name(context_type): doc}) for doc in docs]
        ctx_md = ctx_md[ids]
    else:
...
def main(args):
    ...
    try:
        ...
        def id_fn(metadata):
            return metadata[doc_label_name(lda_v.model.context_type)]
